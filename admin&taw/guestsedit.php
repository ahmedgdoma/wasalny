...
$guests_edit = NULL; // Initialize page object first

class cguests_edit extends cguests {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("guestslist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("guestslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
...
// Create page object
if (!isset($guests_edit)) $guests_edit = new cguests_edit();

// Page init
$guests_edit->Page_Init();

// Page main
$guests_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$guests_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var guests_edit = new ew_Page("guests_edit");
guests_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = guests_edit.PageID; // For backward compatibility

// Form object
var fguestsedit = new ew_Form("fguestsedit");

// Validate form
fguestsedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($guests->user_id->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_travel_id");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($guests->travel_id->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_status");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($guests->status->FldCaption()) ?>");
...
fguestsedit.Form_CustomValidate = 
...
fguestsedit.ValidateRequired = true;
<?php } else { ?>
fguestsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fguestsedit.Lists["x_user_id"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_username","x_phone","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fguestsedit.Lists["x_travel_id"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_travel_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $guests_edit->ShowPageHeader(); ?>
<?php
$guests_edit->ShowMessage();
?>
<form name="fguestsedit" id="fguestsedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="guests">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_guestsedit" class="table table-bordered table-striped">
<?php if ($guests->user_id->Visible) { // user_id ?>
	<tr id="r_user_id">
		<td><span id="elh_guests_user_id"><?php echo $guests->user_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $guests->user_id->CellAttributes() ?>>
<span id="el_guests_user_id" class="control-group">
...
if (is_array($guests->user_id->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($guests->user_id->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<script type="text/javascript">
fguestsedit.Lists["x_user_id"].Options = <?php echo (is_array($guests->user_id->EditValue)) ? ew_ArrayToJson($guests->user_id->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $guests->user_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($guests->travel_id->Visible) { // travel_id ?>
	<tr id="r_travel_id">
		<td><span id="elh_guests_travel_id"><?php echo $guests->travel_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $guests->travel_id->CellAttributes() ?>>
<span id="el_guests_travel_id" class="control-group">
...
if (is_array($guests->travel_id->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($guests->travel_id->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<script type="text/javascript">
fguestsedit.Lists["x_travel_id"].Options = <?php echo (is_array($guests->travel_id->EditValue)) ? ew_ArrayToJson($guests->travel_id->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $guests->travel_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($guests->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_guests_status"><?php echo $guests->status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $guests->status->CellAttributes() ?>>
<span id="el_guests_status" class="control-group">
...
if (is_array($arwrk)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($guests->status->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
...
<?php echo $guests->status->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($guests->id->CurrentValue) ?>">
