...
class cusers extends cTable {
	...
	function __construct() {
		...
		// gender
		$this->gender = new cField('users', 'users', 'x_gender', 'gender', '`gender`', '`gender`', 202, -1, FALSE, '`gender`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['gender'] = &$this->gender;

		// have_car
		$this->have_car = new cField('users', 'users', 'x_have_car', 'have_car', '`have_car`', '`have_car`', 202, -1, FALSE, '`have_car`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->have_car->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->fields['have_car'] = &$this->have_car;

		// car_number
		$this->car_number = new cField('users', 'users', 'x_car_number', 'car_number', '`car_number`', '`car_number`', 200, -1, FALSE, '`car_number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['car_number'] = &$this->car_number;

		// car_model
		$this->car_model = new cField('users', 'users', 'x_car_model', 'car_model', '`car_model`', '`car_model`', 200, -1, FALSE, '`car_model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['car_model'] = &$this->car_model;

		// car_color
		$this->car_color = new cField('users', 'users', 'x_car_color', 'car_color', '`car_color`', '`car_color`', 200, -1, FALSE, '`car_color`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['car_color'] = &$this->car_color;

		// type
		$this->type = new cField('users', 'users', 'x_type', 'type', '`type`', '`type`', 202, -1, FALSE, '`type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['type'] = &$this->type;

		// img
		$this->img = new cField('users', 'users', 'x_img', 'img', '`img`', '`img`', 200, -1, FALSE, '`img`', FALSE, FALSE, FALSE, 'IMAGE');
	...
	function InsertSQL(&$rs) {
		...
		foreach ($rs as $name => $value) {
			if (!isset($this->fields[$name]))
				continue;
			if (EW_ENCRYPTED_PASSWORD && $name == 'phone')
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->have_car->Exportable) $Doc->ExportCaption($this->have_car);
				if ($this->car_number->Exportable) $Doc->ExportCaption($this->car_number);
				if ($this->car_model->Exportable) $Doc->ExportCaption($this->car_model);
				if ($this->car_color->Exportable) $Doc->ExportCaption($this->car_color);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->have_car->Exportable) $Doc->ExportField($this->have_car);
					if ($this->car_number->Exportable) $Doc->ExportField($this->car_number);
					if ($this->car_model->Exportable) $Doc->ExportField($this->car_model);
					if ($this->car_color->Exportable) $Doc->ExportField($this->car_color);
