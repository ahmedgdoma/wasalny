...
class cusers_edit extends cusers {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// phone
			$this->phone->SetDbValueDef($rsnew, $this->phone->CurrentValue, "", $this->phone->ReadOnly || (EW_ENCRYPTED_PASSWORD && $rs->fields('phone') == $this->phone->CurrentValue));
			...
			// have_car
			$this->have_car->SetDbValueDef($rsnew, ((strval($this->have_car->CurrentValue) == "1") ? "1" : "0"), 0, $this->have_car->ReadOnly);

			// car_number
			$this->car_number->SetDbValueDef($rsnew, $this->car_number->CurrentValue, "", $this->car_number->ReadOnly);

			// car_model
			$this->car_model->SetDbValueDef($rsnew, $this->car_model->CurrentValue, "", $this->car_model->ReadOnly);

			// car_color
			$this->car_color->SetDbValueDef($rsnew, $this->car_color->CurrentValue, "", $this->car_color->ReadOnly);
