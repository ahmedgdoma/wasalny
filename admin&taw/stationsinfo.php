...
//
// Table class for stations
//
class cstations extends cTable {
	var $id;
	var $travel_id;
	var $station_name;
	var $longitude;
	...
	function __construct() {
		...
		// id
		$this->id = new cField('stations', 'stations', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// travel_id
		$this->travel_id = new cField('stations', 'stations', 'x_travel_id', 'travel_id', '`travel_id`', '`travel_id`', 19, -1, FALSE, '`travel_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->travel_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['travel_id'] = &$this->travel_id;

		// station_name
		$this->station_name = new cField('stations', 'stations', 'x_station_name', 'station_name', '`station_name`', '`station_name`', 200, -1, FALSE, '`station_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['station_name'] = &$this->station_name;

		// longitude
		$this->longitude = new cField('stations', 'stations', 'x_longitude', 'longitude', '`longitude`', '`longitude`', 200, -1, FALSE, '`longitude`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['longitude'] = &$this->longitude;

		// latitude
		$this->latitude = new cField('stations', 'stations', 'x_latitude', 'latitude', '`latitude`', '`latitude`', 200, -1, FALSE, '`latitude`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['latitude'] = &$this->latitude;

		// created_at
		$this->created_at = new cField('stations', 'stations', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('stations', 'stations', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("stationsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("stationsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "stationsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("stationsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("stationsadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("stationsdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->travel_id->Exportable) $Doc->ExportCaption($this->travel_id);
				if ($this->station_name->Exportable) $Doc->ExportCaption($this->station_name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->travel_id->Exportable) $Doc->ExportField($this->travel_id);
					if ($this->station_name->Exportable) $Doc->ExportField($this->station_name);
