...
//
// Table class for travels
//
class ctravels extends cTable {
	var $id;
	var $user_id;
	var $travel_name;
	var $start_point;
	...
	function __construct() {
		...
		$this->TableVar = 'travels';
		$this->TableName = 'travels';
		...
		// id
		$this->id = new cField('travels', 'travels', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// user_id
		$this->user_id = new cField('travels', 'travels', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 19, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// travel_name
		$this->travel_name = new cField('travels', 'travels', 'x_travel_name', 'travel_name', '`travel_name`', '`travel_name`', 200, -1, FALSE, '`travel_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['travel_name'] = &$this->travel_name;

		// start_point
		$this->start_point = new cField('travels', 'travels', 'x_start_point', 'start_point', '`start_point`', '`start_point`', 200, -1, FALSE, '`start_point`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['start_point'] = &$this->start_point;

		// end_point
		$this->end_point = new cField('travels', 'travels', 'x_end_point', 'end_point', '`end_point`', '`end_point`', 200, -1, FALSE, '`end_point`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['end_point'] = &$this->end_point;

		// capacity
		$this->capacity = new cField('travels', 'travels', 'x_capacity', 'capacity', '`capacity`', '`capacity`', 200, -1, FALSE, '`capacity`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['capacity'] = &$this->capacity;

		// start_time
		$this->start_time = new cField('travels', 'travels', 'x_start_time', 'start_time', '`start_time`', 'DATE_FORMAT(`start_time`, \'%Y/%m/%d\')', 135, 9, FALSE, '`start_time`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->start_time->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['start_time'] = &$this->start_time;

		// passenger_gender
		$this->passenger_gender = new cField('travels', 'travels', 'x_passenger_gender', 'passenger_gender', '`passenger_gender`', '`passenger_gender`', 202, -1, FALSE, '`passenger_gender`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['passenger_gender'] = &$this->passenger_gender;

		// status
		$this->status = new cField('travels', 'travels', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['status'] = &$this->status;

		// created_at
		$this->created_at = new cField('travels', 'travels', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('travels', 'travels', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("travelsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("travelsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "travelsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("travelsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("travelsadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("travelsdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->travel_name->Exportable) $Doc->ExportCaption($this->travel_name);
				if ($this->start_point->Exportable) $Doc->ExportCaption($this->start_point);
				if ($this->end_point->Exportable) $Doc->ExportCaption($this->end_point);
				if ($this->capacity->Exportable) $Doc->ExportCaption($this->capacity);
				if ($this->start_time->Exportable) $Doc->ExportCaption($this->start_time);
				if ($this->passenger_gender->Exportable) $Doc->ExportCaption($this->passenger_gender);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->travel_name->Exportable) $Doc->ExportField($this->travel_name);
					if ($this->start_point->Exportable) $Doc->ExportField($this->start_point);
					if ($this->end_point->Exportable) $Doc->ExportField($this->end_point);
					if ($this->capacity->Exportable) $Doc->ExportField($this->capacity);
					if ($this->start_time->Exportable) $Doc->ExportField($this->start_time);
					if ($this->passenger_gender->Exportable) $Doc->ExportField($this->passenger_gender);
